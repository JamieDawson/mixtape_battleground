...
function App() {
		...
		fetch('/getAllData')
			...
			.then((resp) => {
				const updatedValue1 = {
					...
					album_id_thrid: resp[0].album_id_thrid,
				};
	...
	const onSubmit = (data) => {
		...
		for (const [key, value] of Object.entries(data)) {
			...
			if (value !== '') {
				console.log('Inside if statement for value');
				findAlbumKey = key;
	...
	return (
		<div className='App'>
			3221399452 //woman chilling 3057796265 //cult of mary // 2552716785 Cold
			Cave
			<div className='app__all_albums'>
				...
				<div className='app__album'>
					<BandcampPlayer album={albumIdAll.album_id_thrid} size='large' />
					<form onSubmit={handleSubmit(onSubmit)}>
						<input name='album_id_thrid' ref={register} />
						<input type='submit' />
					</form>
				</div>
			</div>
